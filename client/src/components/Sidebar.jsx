  ...
  return (
    ...
    border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuopen && 'max-md:-translate-x-full'}`}>
      ...
      dark:border-white/20 rounded-md'>
        ...
        placeholder='Search Conversation' className='text-xs placeholder:text-gray-400 outline-none'/>
      </div>
      ...
      <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
        {
          chat.filter((c) => c.messages[0] ? c.messages[0]?.content.
        toLowerCase().includes(search.toLowerCase()) : c.name.toLowerCase().
      includes(search.toLowerCase())).map((c) => (
        <div onClick={() => {navigate('/'); setSelectedChat(c); setIsMenuopen(false)}}
        key={c._id} className='p-2 px-4 dark:bg-[#57317C]/10 border
        border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer 
        flex justify-between group'>
          <div>
            <p className='truncate w-full'>
              {c.messages.length> 0 ? c.messages[0]?.content.slice(0,32): c.name}
            </p>
